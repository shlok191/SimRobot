...
MainWindow::MainWindow(int, char *argv[]) : appPath(getAppPath(argv[0])),
...
bool MainWindow::compileModules(std::vector<float> &positions)
{
  if (compiled)
    return true;
  ...
  for (int i = 0; i < loadedModules.count(); ++i) // note: list of modules may grow while compiling modules
  {
    ...
    if (!loadedModule->compiled)
    {
      loadedModule->compiled = loadedModule->module->compile(positions);
      if (!loadedModule->compiled)
        success = false;
    }
  }
...
void MainWindow::updateViewMenu(QMenu *menu)
{
  ...
  auto addUpdateRateAction = [this](const char *label, int updateRate)
  {
    ...
    connect(action, &QAction::triggered, this, [this, updateRate]
            { setGuiUpdateRate(updateRate); });
  };
...
void MainWindow::updateMenuAndToolBar()
{
  ...
  toolBar->setStyleSheet("QToolBar {padding: 0px 6px 0px 6px;"
                         ...
                         pressed.name(QColor::HexArgb) + "}"
                                                         "QToolButton:checked:hover {background-color: " +
                         checkedHover.name(QColor::HexArgb) + "}"
                                                              "QToolButton:checked:pressed {background-color: " +
  ...
  for (QDockWidget *dockWidget : findChildren<QDockWidget *>())
  {
    ...
    dockWidget->setStyleSheet("QDockWidget {titlebar-close-icon: url(:/Icons/icons8-close" + QString(Theme::isDarkMode(this) ? "-dark" : "") + "-50.png)}"
                                                                                                                                               "QDockWidget::title {text-align: center;"
                                                                                                                                               "padding-" +
                              QString(vertical ? "bottom" : "left") + ": 3px;"
                              ...
                              QString(vertical ? "center bottom" : "left center") + "}"
                                                                                    "QDockWidget::close-button:hover {background: " +
                              hover.name(QColor::HexArgb) + "}"
                                                            "QDockWidget::close-button:pressed {background: " +
                              pressed.name(QColor::HexArgb) + "}");
  }
...
void MainWindow::openFile(const QString &fileName, std::vector<float> &positions)
{
  closeFile();
